...
import RoomCard, { RoomCardP } from './components/RoomCard'
import './App.css'
...
function App() {
  ...
  const profesores = ['Todos', ...new Set(horarios.map(h => h.profesor))].sort()
  ...
  //para vista de profesores
  const profesoresAgrupados = horariosFiltrados.reduce((acc, h) => {
    if (!acc[h.profesor]) acc[h.profesor] = []
    acc[h.profesor].push(h)
    return acc
  }, {})
  ...
  const hayFiltros = carreraFiltro !== 'Todas' || turnoFiltro !== 'Todos' ||
    grupoFiltro !== 'Todos' || diaFiltro !== 'Todos' || salonFiltro !== 'Todos' || profesorFiltro !== 'Todos'
      ...
      <main className="app-main">
        ...
        {vista === 'profesores' && (
          horariosFiltrados.length === 0 ? (
            ...
            <div className="rooms-grid">
              {Object.entries(profesoresAgrupados)
                .sort(([a], [b]) => a.localeCompare(b))
                .map(([profesor, clases]) => (
                  <RoomCardP key={profesor} profesor={profesor} clases={clases} />
                ))
              }
            </div>
          )
